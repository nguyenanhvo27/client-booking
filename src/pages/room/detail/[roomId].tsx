...
function Page({}: Props) {
  ...
  const disableDate =
    query.isSuccess && Array.isArray(query.data?.__hotel__?.__reservations__)
      ? query.data?.__hotel__?.__reservations__?.map((range: Range) => {
          return {
  ...
  return (
    <Layout>
            ...
            <div className="mx-auto max-w-2xl px-4 pt-10 sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-3 lg:grid-rows-[auto,auto,1fr] lg:gap-x-8 lg:px-8 lg:pt-16">
              ...
              {(session?.user as any)?.role === "user" ? (
                <div className="mt-4 lg:row-span-3 lg:mt-0">
                  <h2 className="sr-only">Thông tin</h2>
                  <p className="text-3xl tracking-tight text-gray-900 ">
                    {!isNaN(query?.data?.prize) &&
                      Number(query?.data?.prize).toLocaleString("en-US", {
                        style: "currency",
                        currency: "VND",
                      })}
                    / Ngày
                  ...
                  <div className="sm:col-span-4 mt-5">
                    ...
                    {errors.dateRange && (
                      <p className="text-red-500">Nhập ngày đi hợp lệ</p>
                    )}
                  </div>
                    ...
                    <div className="mt-2">
                      ...
                      {errors.traveller && (
                        <p className="text-red-500">
                          Người ở phải nhỏ hơn hoặc bằng với sức chứa phòng
                        </p>
                      )}
                    </div>
                  </div>
              ...
              <div className="py-10 lg:col-span-2 lg:col-start-1 lg:border-r lg:border-gray-200 lg:pb-16 lg:pr-8 lg:pt-6">
                ...
                <h4 className="my-2">
                  <span className="font-semibold">Giới hạn người</span> :{" "}
                  {query?.data?.capacity}
                </h4>
                <h4 className="my-2">
                ...
                <div className="mt-10">
                  ...
                  <div className="my-4 ">
                    {query?.isSuccess && (
                      <ul
                        role="list"
                        className=" flex list-disc space-y-2 pl-4 text-sm"
                      >
                        {query.data?.__roomType__?.AC && (
                          <React.Fragment>
                            <li className="px-6 text-gray-400 flex">
                              <div className="text-gray-600 flex justify-start items-center gap-3">
                                Máy lạnh:
                                <BsSnow className="text-3xl" color="green" />
                              </div>
                            </li>
                          </React.Fragment>
                        )}
                        {query.data?.__roomType__?.heater && (
                          <React.Fragment>
                            <li className="text-gray-400 flex px-6">
                              <div className="text-gray-600 flex justify-start items-center gap-3">
                                Máy sưởi
                                <GiHotSurface
                                  className="text-3xl"
                        ...
                        {query.data?.__roomType__?.wifi && (
                          <React.Fragment>
                            <li className="text-gray-400 px-6 ">
                              <div className="text-gray-600 flex justify-start items-center gap-3">
                                Wifi
                                <BsWifi className="text-3xl" color="green" />
                              </div>
                            </li>
                          </React.Fragment>
                        )}
                        {query.data?.__roomType__?.parking && (
                          <React.Fragment>
                            <li className="text-gray-400 px-6">
                              <div className="text-gray-600 flex justify-start items-center gap-3">
                                Bãi đỗ xe
                                <FaParking className="text-3xl" color="green" />
                              </div>
                            </li>
                          </React.Fragment>
                        )}
                        {query.data?.__roomType__?.pool && (
                          <React.Fragment>
                            <li className="text-gray-400 px-6">
                              <div className="text-gray-600 flex justify-start items-center gap-3">
                                Hồ bơi
                ...
                <p>
                  <span className="font-semibold text-xl">Mô tả phòng :</span>{" "}
      ...
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl text-semibold">Bình luận:</h1>
        {queryComment.isSuccess ? (
          <Comment data={queryComment.data[0]} total={queryComment.data[1]} />
        ) : (
          <p>Chwua có bình luận nào</p>
        )}
      </div>
    </Layout>
  );
