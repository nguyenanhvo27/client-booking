...
const navigation = [
  { name: "Danh Sách Khách Sạn", href: "/hotel/list", role: "hotelier" },
  {
    name: "Phòng",
...
function Navbar({ session, status }: Props) {
  ...
  return (
    <>
      <header className="absolute inset-x-0 top-0 z-50 bg-blue-800">
        <nav
          className="flex items-center justify-between p-6 lg:px-8"
          aria-label="Global"
        >
          <div className="flex lg:flex-1 lg:justify-start">
            <Link href="/" className="-m-1.5  w-[calc(4rem)] h-[calc(4rem)]">
              <span className="text-yellow-500 font-extrabold text-3xl">
                STU BOOKING
              </span>
            </Link>
          </div>
          ...
          <div className="hidden lg:flex lg:flex-1 lg:justify-end ">
            {status === "authenticated" ? (
              ...
              <div className="flex gap-2 items-center">
                ...
                <VscSignIn className="text-white" />
                <Link href={"/register"}>
                  <p className="text-base font-semibold leading-6 text-white cursor-pointer">
                    Đăng ký
                  </p>
                </Link>
              </div>
